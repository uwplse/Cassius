...
bench/css/%.rkt: get_bench.py get_bench.js
	@ xvfb-run -a -s '-screen 0 1920x10800x24' python2 get_bench.py --name css/$* $(patsubst %,file://%,$(wildcard $(CSSWG_PATH)/css21/$*/*.xht))

bench/alexa/%.rkt: get_bench.py get_bench.js bench/alexa/%.html
	@ xvfb-run -a -s '-screen 0 1920x10800x24' python2 get_bench.py --name alexa/$* file://$(PWD)/bench/alexa/$*.html
