...
(define-constraints layout-definitions
  ...
  (define-fun float-rules ((b Box)) Bool
    ,(smt-let ([p (pflow b)] [flt (flt b)] [vb (vbox b)])
       ...
       ;; collapsing margins, the float is positioned as if it had an
       ;; otherwise empty anonymous block parent taking part in the
       ;; flow. The position of such a parent is defined by the rules
       ;; in the section on margin collapsing.

       ;; CSS 2.1, § 9.5.1, item 1: The left outer edge of a
       ;; left-floating box may not be to the left of the left edge of
       ;; its containing block.
       (=> (is-float/left (float b)) (>= (left-outer b) (left-content p)))
       (=> (is-float/right (float b)) (>= (right-content p) (right-outer b)))

       ;; CSS 2.1, § 9.5.1, item 2: If the current box is
       ;; left-floating, and there are any left-floating boxes
       ;; generated by elements earlier in the source document, then
       ;; for each such earlier box, either the left outer edge of the
       ;; current box must be to the right of the right outer edge of
       ;; the earlier box, or its top must be lower than the bottom of
       ;; the earlier box. SIMPL: either to the right of the previous
       ;; float, or below it.
       ...
       ;; CSS 2.1, § 9.5.1, item 3: The right outer edge of a
       ;; left-floating box may not be to the right of the left outer
       ;; edge of any right-floating box that is next to it. Analogous
       ;; rules hold for right-floating elements. TODO: Currently
       ;; impossible --- see restrictions below

       ;; CSS 2.1, § 9.5.1, item 4: A floating box's outer top may not
       ;; be higher than the top of its containing block. When the
       ;; float occurs between two collapsing margins, the float is
       ;; positioned as if it had an otherwise empty anonymous block
       ;; parent taking part in the flow. The position of such a
       ;; parent is defined by the rules in the section on margin
       ;; collapsing.
       (>= (top-outer b) (top-content p))

       ;; CSS 2.1, § 9.5.1, item 5: The outer top of a floating box
       ;; may not be higher than the outer top of any block or floated
       ;; box generated by an element earlier in the source document.
       ;; CSS 2.1 § 9.5.1, item 6: The outer top of an element's
       ;; floating box may not be higher than the top of any line-box
       ;; containing a box generated by an element earlier in the
       ;; source document. SIMPL: May not be higher than the top of
       ;; the previous float or flow box
       ...
       ;; CSS 2.1, § 9.5.1, item 7: A left-floating box that has
       ;; another left-floating box to its left may not have its right
       ;; outer edge to the right of its containing block's right
       ;; edge. (Loosely: a left float may not stick out at the right
       ;; edge, unless it is already as far to the left as possible.)
       ...
       ;; CSS 2.1, § 9.5.1, item 8: A floating box must be placed as
       ;; high as possible. SIMPL: at its normal position, or the same
       ;; y-position as previous float
       ...
       ;; CSS 2.1, § 9.5.1, item 9: A left-floating box must be put as
       ;; far to the left as possible, a right-floating box as far to
       ;; the right as possible. A higher position is preferred over
       ;; one that is further to the left/right. SIMPL: at the
       ;; left/right or next to an existing floating box
