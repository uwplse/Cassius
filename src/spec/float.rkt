...
(define *exclusion-zone-registers* (make-parameter 4))

(define-constraints exclusion-zones
  (declare-datatypes ()
     ((EZone (ezone (mark Real)
  ...
  (define-fun ez.add ((ez EZone) (top Real) (right Real) (bottom Real) (left Real)) EZone
           ...
           (define (copy-register tpl #:merge merge #:new new)
             (smt-cond
              ...
              [else
               ,(if (= i 0)
                    new
                    `(if (< (,(sformat "ez.y~a" (- i 1)) ez) bottom)
                         ,new
                         (,(sformat tpl (- i 1)) ez))))))
    ...
    (ezone
     ...
     ,@(let ([transfer (λ (i tpl default)
                   (for/fold ([expr `(,(sformat tpl i) ez)])
                     ...
                     `(if (< (,(sformat "ez.y~a" j) ez) y)
                          ,(if (< (+ i j 1) (*exclusion-zone-registers*))
                               `(,(sformat tpl (+ i j 1)) ez)
                               default)
                          ,expr)))])
         (for/reap [sow] ([i (in-range (*exclusion-zone-registers*))])
           (sow (transfer i "ez.y~a" 0))
           (sow (transfer i "ez.l~a" 0))
           (sow (transfer i "ez.r~a" 0))
           (sow (transfer i "ez.l~a?" 'false))
           (sow (transfer i "ez.r~a?" 'false))))))

  (define-fun ez.x ((ez EZone) (y Real) (width Real) (dir Float) (pl Real) (pr Real)) Real
    ...
    ,(for/fold ([x `(if (is-float/left dir) pl pr)]) ([i (in-range (*exclusion-zone-registers*))])
       `(if (< (,(sformat "ez.y~a" i) ez) y)
            (if (is-float/left dir)
                (if (,(sformat "ez.l~a?" i) ez) (,(sformat "ez.l~a" i) ez) pl)
                (if (,(sformat "ez.r~a?" i) ez) (,(sformat "ez.r~a" i) ez) pr))
            ,x)))

  (define-fun ez.level ((ez EZone) (width Real) (height Real) (pl Real) (pr Real) (y Real)) Real
    ;; width and height are the dimensions of the box
    ...
    ,(let loop
       ...
       (match* (conditions results)
        ...
        [((cons test conditions) (cons result results))
         `(if ,test ,result ,(loop conditions results))])))

  (define-fun ez.can-add ((ez EZone) (top Real)) Bool
